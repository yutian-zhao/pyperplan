from pyperplan.search import a_star, searchspace
...
def test_ordered_node_weighted_astar4():
    h_val = h4(root4)
    assert a_star.ordered_node_weighted_astar(5)(root4, h_val, 0) == (75, 15, 0, root4)


def test_ordered_node_greedy_best_first1():
    h_val = h1(root1)
    assert a_star.ordered_node_greedy_best_first(root1, h_val, 0) == (0, 0, 0, root1)


def test_ordered_node_greedy_best_first2():
    h_val = h2(root2)
    assert a_star.ordered_node_greedy_best_first(root2, h_val, 0) == (5, 5, 0, root2)


def test_ordered_node_greedy_best_first3():
    h_val = h3(root3)
    assert a_star.ordered_node_greedy_best_first(root3, h_val, 0) == (4, 4, 0, root3)


def test_ordered_node_greedy_best_first4():
    h_val = h4(root4)
    assert a_star.ordered_node_greedy_best_first(root4, h_val, 0) == (15, 15, 0, root4)


def test_astar_search1():
    """ The initial state is the goal state, so the plan is an empty list"""
    assert (
        a_star.astar_search(task1, h1, make_open_entry=a_star.ordered_node_astar)[0] == []
    )
...
def test_astar_search2():
    ...
    assert (
        len(a_star.astar_search(task2, h2, make_open_entry=a_star.ordered_node_astar)[0])
        == 3
    )
...
def test_astar_search3():
    assert (
        len(a_star.astar_search(task3, h3, make_open_entry=a_star.ordered_node_astar)[0])
        == 4
    )
...
def test_astar_search4():
    ...
    assert (
        a_star.astar_search(task4, h4, make_open_entry=a_star.ordered_node_astar)[0]
        is None
    )
